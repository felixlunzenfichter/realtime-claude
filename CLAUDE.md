...
## Protocol: [ProtocolName] ([Conformances])
- property: Type
...
## [Class/Struct]: [Name] ([modifiers], [conformances])

### Constants
- name: Type = value → usedIn(): newValue

### Properties
- name: Type = default → mutator(): newValue

### Computed Properties
- name: Type → uses: property1, property2

### Functions
- functionName(params) → internalCall(), ExternalClass.externalMethod(), property=value, if condition: action
- leafFunction() → (leaf)
*/
```

**Rules:**
- **Computed Properties**: `→ uses:` shows dependencies
- **Functions**: Internal calls `call()`, External `Class.method()`
