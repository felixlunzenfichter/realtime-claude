...
//
// Usage:
//   const myFunction = contract('myFunction', (arg1, arg2) => {
//       // pure logic, no contract clutter
//       return result;
//   });
//
// Contract definitions go in CONTRACTS object below.
// Pre/post/invariants are checked automatically.
// ============================================================================
...
const CONTRACTS = {
    ...
    // functionName: {
    //     pre: [
    //         [args => args[0] !== null, "first arg required"],
    //     ],
    //     post: [
    //         [(result, args) => result !== null, "must return value"],
    //     ],
    //     inv: [
    //         [() => someGlobalCondition, "invariant description"],
    //     ]
    // }
};
...
function contract(name, fn) {
    return (...args) => {
        ...
        c.pre?.forEach(([check, msg]) => {
            if (!check(args)) {
                console.error(`❌ PRE[${name}]: ${msg}`);
                throw new Error(`Precondition failed [${name}]: ${msg}`);
            }
        });

        c.inv?.forEach(([check, msg]) => {
            if (!check()) {
                console.error(`❌ INV[${name}] before: ${msg}`);
                throw new Error(`Invariant failed [${name}]: ${msg}`);
            }
        });

        const result = fn(...args);

        c.post?.forEach(([check, msg]) => {
            if (!check(result, args)) {
                console.error(`❌ POST[${name}]: ${msg}`);
                throw new Error(`Postcondition failed [${name}]: ${msg}`);
            }
        });

        c.inv?.forEach(([check, msg]) => {
            if (!check()) {
                console.error(`❌ INV[${name}] after: ${msg}`);
                throw new Error(`Invariant failed [${name}]: ${msg}`);
            }
        });
